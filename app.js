...
function windowBounds(){
  // Business rule: the Optimal Execution Window is bounded by the START dates
  // of the configured key rows. This intentionally avoids using the later
  // iteration end date (the extra 02/08/30-type boundary).
  const a=DATA[CFG.startRow-1], b=DATA[CFG.endRow-1];
  const pa=parseIteration(a?.iteration), pb=parseIteration(b?.iteration);
  return {start:pa?.start||a.date, end:pb?.start||b.date};
}
...
function render(){
  ...
  const x=d=>m.l+((+d-x0)/(x1-x0))*pw,y=v=>m.t+(max-v)/(max-min)*ph,base=y(min),win=windowBounds();
  ...
  if($('process').checked)proc(x,m,W,H,x0,x1,win);
  ...
  if(selected>=0&&DATA[selected]){
    ...
    showPop(d,sx,Math.min(sy,ly),W,H,m);
  }else popup.hidden=true;
}

function proc(x,m,W,H,x0,x1,win){
