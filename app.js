...
import { drawProcess } from "./process.js?v=20260922-liveprocess2";
...
function render(){
  ...
  if($('process').checked)drawProcess({data:DATA,x,m,W,H,x0,x1,win,leaseExpiration:getLeaseExpiration(),svg,ns,txt,scheduleEl:$('schedule')});
  ...
  if(+now>=x0&&+now<=x1){
    const tx=x(now),exp=getLeaseExpiration();
