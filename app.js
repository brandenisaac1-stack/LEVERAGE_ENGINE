...
function pick(fs,cands,req=false){const m=new Map();for(const f of fs){if(f?.name)m.set(String(f.name).toLowerCase(),f.name);if(f?.alias)m.set(String(f.alias).toLowerCase(),f.name)}for(const c of cands){if(m.has(c.toLowerCase()))return m.get(c.toLowerCase())}if(req)throw new Error('Required field missing: '+cands.join(' / '));return null}function val(a,k){return fields[k]?a[fields[k]]:null}
...
function render(){
  ...
  const win=windowBounds(),focusMode=!!$('restructure')?.checked,focus=focusMode?restructureFocusBounds(DATA):null;if(focusMode&&!focus)throw new Error('Early Restructure focus requires at least two valid restructure_date_breakout values.');let P=shapedSeries(DATA,win);const W=wrap.clientWidth,H=wrap.clientHeight,m={l:58,r:20,t:68,b:32};
  ...
  // Execution window remains the contextual north star in BOTH modes.
  if(!focusMode){
    ...
    const cx=(m.l+W-m.r)/2, span=Math.min(330,(W-m.l-m.r)*.34), ax1=cx-span/2,ax2=cx+span/2,ay=m.t+27;
    txt(cx,m.t+10,'OPTIMAL EXECUTION WINDOW','t1','middle');
    svg.appendChild(ns('line',{x1:ax1,y1:ay,x2:ax2,y2:ay,class:'windowArrow'}));
    svg.appendChild(ns('path',{d:`M ${ax1} ${ay} l 8 -5 M ${ax1} ${ay} l 8 5 M ${ax2} ${ay} l -8 -5 M ${ax2} ${ay} l -8 5`,class:'windowArrow'}));
