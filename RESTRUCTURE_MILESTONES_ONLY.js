...
function focusedMilestones({data,x,m,W,svg,ns,txt,focusBounds}){
  ...
  const start=new Date(+focusBounds.start),end=new Date(+focusBounds.end);
  ...
  for(const d of data){
    ...
    if(dt instanceof Date&&Number.isFinite(+dt)&&+dt>=+start&&+dt<=+end){
      if(!dates.some(v=>+v===+dt))dates.push(new Date(+dt));
    }
  }
  dates.sort((a,b)=>a-b);
  ...
  const seen=new Set();
  for(const d of data){
    const phase=String(d.restructureActionPhase??'').trim();
    if(!phase)continue;
    const key=phase.toUpperCase();
    if(seen.has(key))continue;
    seen.add(key);phases.push(phase);
  }
  ...
  const masterY=m.t+118,masterH=14,phaseY=masterY+38,phaseH=30;
  ...
  for(let i=0;i<count;i++){
    ...
    if(+b<=+a)continue;
    const x1=x(a),x2=x(b),w=Math.max(2,x2-x1),c=colors[i%colors.length];
    ...
    const title=txt((x1+x2)/2,phaseY+12,phase,'','middle');
    title.setAttribute('fill',c);title.setAttribute('font-size',w<210?'8':w<320?'9':'10');title.setAttribute('font-weight','700');
    const datesText=txt((x1+x2)/2,phaseY+25,`${fmtDate(a)} – ${fmtDate(new Date(+b-86400000))}`,'','middle');
    datesText.setAttribute('fill','#9fb9cd');datesText.setAttribute('font-size','8');
  ...
  svg.appendChild(ns('line',{x1:right,y1:masterY-4,x2:right,y2:phaseY+phaseH+5,stroke:colors[(count-1)%colors.length],'stroke-width':'1','stroke-dasharray':'3 3',opacity:'.65'}));
}
